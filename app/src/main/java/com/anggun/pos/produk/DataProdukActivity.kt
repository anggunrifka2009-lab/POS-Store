...
class DataProdukActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.produkList.observe(this) { list ->
            val adapter = ProdukAdapter(list)
                        ...
                        .setItems(options) { _, which ->
                            when (which) {
                                0 -> { // Edit
                                    ...
                                    intent.putExtra("DATA_PRODUK", produk)
                                    startActivity(intent)
                                }
                                1 -> { // Hapus
                                    AlertDialog.Builder(this@DataProdukActivity)
    ...
    private fun init() {
        ...
        fabTambah = findViewById(R.id.fabTambahp)
        etSearch = findViewById(R.id.etSearchPro)
        ivKembalip = findViewById(R.id.ivKembalip)
    }
    ...
    private fun ambilKategori() {
        ...
        kategoriRef.get().addOnSuccessListener { snapshot ->
            ...
            for (data in snapshot.children) {
                ...
                if (namaKategori.isNotEmpty() && status == "Aktif") {
                    listKategori.add(namaKategori)
                }
            }

            val adapter = FilterKategoriAdapter(listKategori)
            rvKategoriFilter.adapter = adapter

            adapter.setOnItemClickListener(object : FilterKategoriAdapter.OnItemClickListener {
                override fun onItemClick(kategori: String) {
                    if (kategori == "Semua") {
                        viewModel.getData()
                    } else {
                        viewModel.filterKategori(kategori)
                    }
                }
            })
        }
